...
export default function MeusPedidosCliente() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from('orders')
        .select('id, code, service, scheduled_at, minutes, status, price_cents, created_at')
  ...
  return (
    <main className="container-app flex flex-col gap-6 py-10">
          ...
          <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 sem-barra sm:mx-0 sm:flex-col sm:overflow-visible sm:px-0 sm:pb-0">
            <button
              ...
              className={`shrink-0 rounded-full px-4 py-2 text-left text-sm font-bold transition sm:rounded-xl ${
                filtro === 'todos' ? 'bg-tinta-solida text-white' : 'bg-tinta-5 text-tinta-70 hover:text-tinta'
              }`}
            >
              Todos
            </button>
            {statusFiltraveis.map((s) => (
              <button
                ...
                onClick={() => setFiltro(s)}
                className={`shrink-0 rounded-full px-4 py-2 text-left text-sm font-bold transition sm:rounded-xl ${
                  filtro === s ? 'bg-tinta-solida text-white' : 'bg-tinta-5 text-tinta-70 hover:text-tinta'
                }`}
              >
