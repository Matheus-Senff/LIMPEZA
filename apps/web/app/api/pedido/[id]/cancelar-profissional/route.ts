...
/**
 * Cancelamento pelo profissional: sem taxa nenhuma pro cliente (quem
 * desistiu foi o profissional), então o estorno é sempre do valor cheio
 * já pago.
 */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: cancelado } = await cliente.rpc('fn_cancelar_pedido_profissional', { p_order_id: id, p_motivo: motivo });
  if (!cancelado) {
  ...
  const { data: pedido } = await servico.from('orders').select('price_cents').eq('id', id).maybeSingle();
  if (!pedido) return NextResponse.json({ cancelado: true });

  const { reembolsado, erro } = await estornarPagamentoPedido(servico, id, pedido.price_cents);
